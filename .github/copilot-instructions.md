...
- Current work is sandbox proof-of-concept development and production-aligned portfolio experimentation; it is not production infrastructure work.
- Read `README.md` for the current human-readable status and pivot ledger. Use `AGENT_CONTEXT.json` as the compact machine-readable mirror.
- Read `AGENT_CONTEXT.json` after `BOOTSTRAP.md` when it exists; it is the compact current-state context for all agents.
- Read `AGENT_OPERATING_MODEL.md` after `BOOTSTRAP.md` and `AGENT_CONTEXT.json` so the shared role split and drift rules stay consistent.
...
- Before substantive code suggestions or reviews, read `skills/project-coding-preferences/SKILL.md` and apply its shared implementation defaults.
