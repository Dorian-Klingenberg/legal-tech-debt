...
def apply_verdicts(findings: list[dict], verdicts_data: dict) -> list[dict]:
    ...
    for f in findings:
        ...
        f["_carrier"] = "KNIC" if "KNIC" in f.get("source_id", "") else "KFBM"
        confirmed.append(f)
    return confirmed
...
def build_findings_table(confirmed: list[dict], comparison: dict) -> str:
...
def build_report(
    ...
    lines += [
        ...
        "",
        build_findings_table(confirmed, comparison),
        "",
        "---",
        "",
...
def main() -> None:
    ...
    confirmed = apply_verdicts(findings, verdicts_data)
    ...
    report = build_report(intro, comparison, confirmed, pattern_narratives, closing, anchors_data=anchors_data)
